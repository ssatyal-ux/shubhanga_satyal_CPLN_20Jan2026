...
demographic_data <- get_acs(
  ...
  mutate(
    ...
    pop_65plusM =
      sqrt(
        aM^2 + bM^2 + cM^2 + dM^2 + eM^2 + fM^2 +
        gM^2 + hM^2 + iM^2 + jM^2 + kM^2 + lM^2
      )
  ) %>%
  select(
    GEOID,
...
identify_vulnerability <- join_tract_boundaries %>%
  mutate(
    plus_pct = (pop_65plusE / total_populationE) * 100,
    category = case_when(
      median_incomeE < 55924 & plus_pct > 20 ~ "Vulnerable",
      TRUE ~ "Not Vulnerable"
    )
  )
...
**I chose my projection because it is the apt projection for Pennsylvania South.**
...
new_spatial_join <- underserved %>%
  ...
  summarise(
    ...
    total_vul_pop = sum(total_populationE)
  ) 
 glimpse(new_spatial_join)
...
ggplot() +
  geom_sf(data = pa_counties, fill = "lightblue", color = "lightblue") +
  geom_sf(data = underserved %>% filter(serv_under_or == "underserved"), 
          aes(fill = dist_near_hos), color = "lightblue") +
  scale_fill_gradient(low = "white", high = "black", name = "Distance (miles) from nearest hospital") +
  theme_void()
