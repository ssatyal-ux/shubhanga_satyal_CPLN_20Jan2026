...
car_data <- read_csv("car_sales_data1.csv")
```
...
- Rows: 50,000
- Columns: 7
- Variable types: 2, chr and dbl
- Problematic names: 3, because they need backticks.
...
**Your answer:** If we are talking about the first 'head' then yes there is a difference in that it only prints the first 6 rows of the database vs the data frame prints all 50,000 wioth multiple page options. 
...
**Your answer:** Backticks make it an improper name to a proper workable format. R syntax requires that these names have no gaps. The backticks basically correct that fault. 
...
car_data <- add_column(car_data, age= 2025 - car_data$year)
names(car_data)
select(car_data,age)

# Create a mileage_per_year column  
car_data <- add_column(car_data, mileage_per_year = car_data$Mileage/car_data$age)
select(car_data,mileage_per_year)
...
car_data <- mutate( car_data, price_category = case_when(
    Price < 15000 ~ "budget",
...
filter(car_data,price_category == 'high' & Mileage < 5000 )
```
...
avg_mil_by_ft <- car_data %>%
  group_by(car_data$'Fuel type') %>%
